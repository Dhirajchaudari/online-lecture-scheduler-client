...
const Instructor: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      try {
        const response = await sdk.GetAllInstructors();
        if (response && response.getAllInstructors) {
          const formattedUsers = response.getAllInstructors.map((user) => ({
            ...user,
            createdAt: formatDate(user.admin?.createdAt),
            updatedAt: formatDate(user?.admin?.updatedAt),
          }));
          ...
          setMembers(
            formattedUsers.map((el) => ({
              ...
              phone: el?.admin?.phone,
              createdAt: el?.admin?.createdAt,
              updatedAt: el?.admin?.updatedAt,
              numberOfResetPassword: el?.admin?.numberOfResetPassword,
              role: el?.admin?.role,
            }))
          );
        }
  ...
  const handleDeleteAdmin = async () => {
    ...
    try {
      setBtnLoading(true);
      await sdk.DeleteInstructor({ id: adminToDelete.toString() });
      ...
      >
        <form onSubmit={handleSubmit(handleAddAdmin)}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              {...register("phone", {
                ...
                minLength: {
                  value: 10,
                  message: "Phone must be at least 10 digit .",
                },
              })}
              className="input input-primary"
            />
          ...
          <div className="flex justify-end mt-4">
            <CButton variant={ButtonType.Primary}>Add Instructor</CButton>
          </div>
        </form>
      </ReusableModal>
      <ReusableModal
